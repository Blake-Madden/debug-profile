/**@addtogroup Profiling
   @brief Macros used for performance analysis. Compile with the symbol `ENABLE_PROFILING` to enable.
   @details The macros included in this library can track the performance times for either entire functions or specific blocks of code.
...
/**
\def PROFILE()
    Profiles the current function and will write to the profile data when the function completes.
...
\def PROFILE_SECTION_START(section_name)
    Profiles a section of code. A unique label describing the code section should be passed here.
    The profiling will stop when the code section goes out of scope. @sa PROFILE_SECTION_END
...
\def SET_PROFILER_REPORT_PATH(path)
    Sets the @c path to where the profile report will be written.
    This is a tab-delimited report containing the following columns:
    - Function name
    - Times calls
...
\def DUMP_PROFILER_REPORT()
    Outputs all of the current profile information.
    This will automatically happen at the exit of the program, but can be explicitly called via this macro at any time.
*/
/** @} */
...
/* The standard __func__ macro doesn't include the name of the class for member functions,
   so it isn't as useful as it could be. Try to use more descriptive macros (if available) first.*/
    ...
    #define SET_PROFILER_REPORT_PATH(x) ((void)0)
